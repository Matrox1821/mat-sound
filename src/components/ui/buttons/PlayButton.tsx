...
export const PlayButton = ({
  ...
  useRefillUpcoming({ refillUpcoming, currentTrack: !!currentTrack });
  ...
  const handleClick = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (playingFromData?.from === playingFrom.from && tracks[0].id === currentTrack?.id) {
      if (isPlaying) {
        pause();
      } else {
        play();
  ...
  return (
    <button
      onClick={handleClick}
      className={`${tracks[0].song ? "cursor-pointer" : "cursor-not-allowed"} opacity-0 play-button group-hover:opacity-100! z-20 absolute text-white rounded-full p-2 transition-colors ${className}`}
    >
