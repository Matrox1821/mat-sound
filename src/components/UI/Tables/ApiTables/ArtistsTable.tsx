...
export default function ArtistsTable({
  ...
  data: Promise<{ name: string; id: string; avatar: JsonValue }[] | undefined>;
  rows?: string;
}) {
  ...
  const fillEmptyRows = (
    ...
    for (let i = 0; i < emptyCount; i++) {
      filled.push({ name: "", id: `empty-${i}`, avatar: { sm: "", md: "", lg: "" } }); // id único para evitar keys duplicadas
    }
  ...
  const parsedArtists = filledArtists.map(
    ({ id, name, avatar }: { id: string; name: string; avatar: JsonValue }) => {
      return { id, name, avatar: avatar as { sm: string; md: string; lg: string } };
    }
  );
  ...
  const imageBodyTemplate = (artist: any) => {
    if (artist.avatar.sm === "") return <div className="h-[46px]" />;
    return (
      <Image
