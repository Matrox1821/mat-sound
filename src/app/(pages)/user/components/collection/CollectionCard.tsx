...
export function CollectionCard({ element }: { element: UserCollection }) {
  const playlistImage =
    element.type === "playlists" && (element.cover?.md || element.tracksCover[0].md);
  const image = playlistImage || (element.cover && element.cover.md);
  const { data } = usePalette(image || "");
  return (
    <li className="w-full h-68 relative">
      <Link href={`/${element.type}/${element.id}`} className="w-full h-full relative">
        <div className={`w-full h-50 flex items-center justify-center rounded-md relative`}>
          ...
          <div
            className={`w-full h-50 flex items-center justify-center rounded-lg relative bg-background-900 z-10 ${element.type === "playlists" ? "playlist-shadow" : ""}`}
            style={{ backgroundColor: data.darkMuted }}
          >
